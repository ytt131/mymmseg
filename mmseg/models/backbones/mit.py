...
class MixFFN(BaseModule):
    ...
    Args:
        ...
        feedforward_channels (int): The hidden dimension of FFNs.
            Defaults: 1024.
        act_cfg (dict, optional): The activation config(激活配置) for FFNs.
    ...
    def __init__(self,
        ...
        #build_dropout 从mmcv.cnn.bricks.drop导入
        self.dropout_layer = build_dropout(
            dropout_layer) if dropout_layer else torch.nn.Identity()

    def forward(self, x, hw_shape, identity=None):
        out = nlc_to_nchw(x, hw_shape)#从文本顺序变成图像顺序(批量大小，序列长度，通道数)->(批量大小，通道数，宽，高)
        out = self.layers(out)
        out = nchw_to_nlc(out)
        if identity is None:
            identity = x
...
class EfficientMultiheadAttention(MultiheadAttention):
    ...
    def forward(self, x, hw_shape, identity=None):
        ...
        # ``torch.nn.MultiheadAttention`` is (num_query, batch,embed_dims), 
        # We should adjust the shape of dataflow from
        # batch_first (batch, num_query, embed_dims) to num_query_first(num_query ,batch, embed_dims), 
        # and recover ``attn_output``from num_query_first to batch_first.
        if self.batch_first:
            #第0个元素和第1个元素互换位置
